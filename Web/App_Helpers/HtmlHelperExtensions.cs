...
namespace Web.App_Helpers
{
    public static class HtmlHelperExtensions
    {
        ...
        public static IHtmlString RouteFromRequest(this HtmlHelper helper, string value, string attribute, HttpRequestBase request, UriType uriType)
        {
            var currentController = request.Path;
            var currentAction = request.Path;
            ...
            switch (uriType)
            {
                case UriType.Path:
                    ...
                    var hasAction = value.Equals(currentAction, StringComparison.InvariantCultureIgnoreCase);
                    return hasAction || hasController ? new HtmlString(attribute) : new HtmlString(string.Empty);
                case UriType.QueryString:
                    foreach (string key in request.QueryString.Keys)
                    {
                        var queryValue = request.QueryString.Get(key);
                        return queryValue == value ? new HtmlString(attribute) : new HtmlString(string.Empty);
                    }
